...
void ordenarListaPersonas(Lista lista){
    ...
    for ( int i = 0; i<lista->tamanio; i++){
        ...
        paradoEn = lista->primero;

        for ( int j = 0 ; j<-1+lista->tamanio; j++){

            Persona ubicacion = * ((Persona*)paradoEn->data);
            Persona siguiente = * ((Persona*)paradoEn->proximo->data);

            if ( getPeso(ubicacion)< getPeso(siguiente)){

                aux = paradoEn->data;
                paradoEn->data = paradoEn->proximo->data;
                paradoEn->proximo->data = aux;



            }
            paradoEn = paradoEn->proximo;
